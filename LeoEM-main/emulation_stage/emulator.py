...
class MyTopo( Topo ):
    def __init__( self ):
        ...
        for i in range(intermediate_hop_num - 1):
            self.addLink('s%s' % i, 's%s' % (i + 1), bw = bent_pipe_link_bandwidth, delay = unitialized_bent_pipe_delay, loss = Loss, max_queue_size=switch_queue_size)
        # connect the user device and their dish together (the delay should be negligible)
        self.addLink("h1", "s0", bw = bent_pipe_link_bandwidth, delay = unitialized_bent_pipe_delay, loss = Loss, max_queue_size=switch_queue_size)
        self.addLink("h2", "s%s" % (intermediate_hop_num - 1), bw = bent_pipe_link_bandwidth, delay = unitialized_bent_pipe_delay, loss = Loss, max_queue_size=switch_queue_size)
...
def main():
    ...
    # For sending commands to the hosts
    #h2.cmd('iperf -s -i 0.2')
    #h1.cmd('iperf -i 0.2 -t 1000000 -c 10.0.1.102')

                   # Used to null it
    print("bandwitdth", bent_pipe_link_bandwidth)
    print("loss", Loss)
    cli = CLI(net, script='pre_commands.sh')
    cli.do_xterm("h2")
    time.sleep(15)
    cli.do_xterm("h1")
